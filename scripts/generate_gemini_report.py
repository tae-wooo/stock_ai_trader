...
def main():
    ...
    stock_code = input("Gemini 리포트를 생성할 종목코드 입력: ").strip()
    ...
    total_score = calculate_total_score(price_score, news_score, disclosure_score)
    ...
    print()
    print(f"{target_stock.name}({target_stock.code}) Gemini 리포트 생성 중...")
    print("=" * 70)
    ...
    output_path = output_dir / f"{now}_{target_stock.code}_{safe_name}_gemini.txt"
